...
if not os.path.exists(DB_FILE):
    ...
    if menu == "서론: 문제 정의":
        ...
        st.subheader("가설: 노인 운전은 그들의 이동권을 대체할 대중교통 인프라가 부족하기 때문이다.")
        st.subheader("연구 목적: 본 프로젝트는 지역별 면허 반납율과 대중교통 이용 데이터의 상관관계를 분석하여 정책이 성공하기 위해 선행되어야 할 구조적 교통 환경을 도출하고자 한다.")
