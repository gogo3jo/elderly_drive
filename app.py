...
if not os.path.exists(DB_FILE):
    ...
    # --- [섹션 2: 연령별 사고율 분석] ---
        ...
        df1['group'] = df1['age_group'].apply(
            lambda x: '고령층(65세 이상)' if x == '65세 이상' else '기타 연령'
        )

        fig1 = px.bar(
            df1, x='age_group', y='accident_rate', color='group', 
            color_discrete_map={'고령층(65세 이상)': 'crimson', '기타 연령': 'lightgray'}
        )
    ...
    # --- [섹션 5: 결론 및 정책 제언] ---
        ...
        st.markdown("""
        ### 1. 데이터 분석을 통한 결론 
        본 프로젝트는 고령 운전자 사고 위험을 단순히 '고령화로 인한 인지능력 저하'라는 개인적 요인으로만 설명할 수 없으며, 이동권과 교통환경이 복합적으로 작용하는 사회적 문제임을 통계적으로 확인하였다. 현재 면허 자진반납 제도는 2019년부터 전국적으로 시행되고 있으나, 그 운영 실태를 살펴보면 개선점이 명확하다. 지원 대상 연령은 지자체별로 만 65세부터 70세까지 상이하며, 보상 또한 소액 선불형 교통카드나 지역사랑상품권 등의 일회성 지원이 주를 이룬다. 
        
        분석 결과 고령 운전자의 사고 위험도는 실제 비고령 운전자보다 높으나 면허 반납율의 지역적 편차는 대중교통 이용 지표와 밀접한 양의 상관관계를 보인다. 즉 고령 운전자가 운전대를 놓지 못하는 것은 단순한 고집이 아니라 대중교통 인프라가 갖춰지지 않은 지역에서 면허 반납이 곧 이동권의 단절을 의미하기 때문이다. 따라서 지자체별로 상이한 반납 기준과 일회성 보상 방식으로는 지역 간 이동권 격차를 해소하고 자발적 반납을 유도하는 데 한계가 있으며, 향후 정책의 성공은 지역별 환경을 고려한 촘촘한 대체 이동 수단 제공에 달려 있다. 
        """)
        st.write("")
        st.markdown("""    
        ### 2. 정책 제언
        **(1) 지역 맞춤형 이동권 보장**
        - 현재의 획일적인 보상(교통카드/상품권)은 교통망이 갖춰진 도시 지역에는 적합하나, 농어촌 사각지대에는 한계가 있다. 따라서 지자체별 교통 환경을 고려해 대중교통 취약 지역에는 금전 지원 대신 수요응답형 교통(DRT, 마을택시 등) 이용권을 연계하여 실질적인 이동 수단을 보장해야 한다. 
        
        **(2) 사회적 예우 중심의 면허 반납 재정의**
        - 일본의 ‘운전 경력 증명서’를 벤치마킹하여 면허 반납을 개인의 희생이 아닌 교통 약자로서의 사회적 예우로 재정의하는 인식 변화가 필요하다. 이를 통해 공공시설 이용료 감면, 지역 상점 연계 할인 등 실질적 할인을 제공해 사회적 고립을 예방해야 한다. 

        **(3) 사고 예방 정책**
        고령운전자의 인지, 반응 능력을 보완하기 위해 첨단 운전자 보조장치(ADAS) 장착을 지원하고, 도로 표지판 및 신호 체계 등 고령자 친화적 도로 환경을 구축해야 한다. 또한 자동긴급제동장치(AEBS) 등이 탑재된 차량에 대해 표시 부착을 유도하여 주변 운전자의 방어운전을 유도하는 상호 배려형 교통 문화를 조성해야 한다.  
        """)
        st.write("")
        st.markdown("""
        ### 3. 연구 한계 및 후속 연구 방향 
        (1) 연구 한계
        - 본 프로젝트는 지자체별 통계 기반의 거시적 분석으로, 개인의 실제 이동 경로, 심리적 요인, 해당 지역의 상세 대중교통 배차 간격 등을 세밀하게 반영하지 못했다는 한계가 있다. 
        (2) 후속 연구 방향 및 미래 전망 
        - 미시적 최적 모델 연구: 향후 개인별 이동 데이터 혹은 실시간 대중교통 접근성을 활용한 미시적 분석으로 반납 후 삶의 질까지 고려한 이동권 보장 최적 모델을 연구할 필요가 있다. 
        - 자율주행 기술을 활용한 솔루션: 기술적 진보는 이동 수단 한계의 돌파구가 될 수 있다. 자율주행 교통수단을 대중교통 사각지대에 보급함으로써 운전면허를 반납한 고령자가 집에서 정류장까지, 혹은 병원까지 이동하는 거리를 기술적으로 해결해 줄 수 있다. 
        """)
        st.write("")
        st.markdown("""
        ### 4. 활용 데이터 
        - **경찰청 (2020).** 운전면허소지자 연령별 현황(2020년 12월 말 기준). 공공데이터포털.
        - **김용렬, 이정민, 우성휘 (2019).** 고령 운전자 운전면허 관리 강화에 대한 농촌의 의견. KREI 현안분석, 제62호, 한국농촌경제연구원. https://www.krei.re.kr/attach/pdf/PRI062.pdf
        - **도로교통공단 (2020).** 연령대별 가해운전자 교통사고 통계. 교통사고분석시스템(TAAS).
        - **도로교통공단 (2025).** "고령운전자도 '운전능력 평가 강화' 원한다" - 공단 인식조사 결과 보도자료. (2025. 05. 15). https://www.koroad.or.kr/main/board/6/302823/board_view.do
        - **통계청 (2024).** 시도별 고령인구비율. KOSIS 국가통계포털.
        - **한국교통안전공단 (2024).** 시도별 대중교통 이용인원. 교통카드 빅데이터 통합정보시스템.
        - **한국사회과학자료원 (2017).** 노인인권에 대한 인식 및 실태조사: 청장년. KOSSDA.
        - **행정안전부 (2024).** 시도 경찰청별 고령운전자 자진반납 현황. 공공데이터포털.
        - 동아일보 (2025. 06. 29.). 시청역 참사 1년, 고령운전 사고 여전… "면허 반납-이동권 보장 필요". https://www.donga.com/news/Society/article/all/20250629/131905731/2
        - 브라보마이라이프 (2024. 10. 02.). 고령자 운전사고 문제, '노인왕국' 일본이 찾은 지혜는?. https://bravo.etoday.co.kr/view/atc_view/15731
        - 연합뉴스 (2025. 07. 01.). 고령운전자 사고 역대 최다. https://www.yna.co.kr/view/PYH20250701135800013
        - 오마이뉴스 (2024. 07. 16.). '급발진'은 막고 고령운전 배려하는 일본, 이렇게 한다. https://www.ohmynews.com/NWS_Web/View/at_pg.aspx?CNTN_CD=A0003043798
        - 한겨레 (2025. 07. 28.). 80대가 몬 차량에 치여 12살 사망…급증세 '고령 운전사고' 어쩌나. https://www.hani.co.kr/arti/area/capital/1210329.html
        """)
        st.balloons()